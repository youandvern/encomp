from application.calcscripts.process.basedefinitions import CalculationTitle, DescriptionHead, Assumption, \
    ...
    DeclareVariable, DeclareTable, CalcTable, CalcVariable, BodyText, BodyHeader, HeadCollection, SetupCollection, \
    CalcCollection, FootCollection, AssumCollection
...
def create_calculation(updated_input={}):
    ...
    default_bars_table = [['bar_id', 'As', 'db', 'x', 'y'], ["bar1", .44, .75, 3, 22.125],
                          ["bar2", .44, .75, 6, 22.125], ["bar3", .44, .75, 9, 22.125]]
    ...
    B1 = CalcVariable('\beta _1', MAX(0.65, MIN(0.85, 0.85 - 0.05 * BRACKETS((fc - 4000) / 1000))), '', 'Equivalent rectangular compressive stress block depth ratio',
    ...
    # solve for c
    c_assume = 0.001
    c_change = h.value / 2  # assume c is within the section (flexure only)
    c_change_factor = 2
    c_last_change_up = False
    ...
    while not c_solved:
        ...
        for bar in bars_val:
            ...
            Ptot += bar[1] * fsi
        if Ptot > P_tolerance:  # c needs to increase
            if not c_last_change_up:  # last change was a decrease
                c_last_change_up = True
            c_assume += c_change
        elif Ptot < -1 * P_tolerance:  # c needs to decrease
            if c_last_change_up:  # last change was an increase
                c_last_change_up = False
            c_assume -= c_change
        else:
            c_solved = True
        c_change = c_change / c_change_factor
    ...
    et_max = CalcVariable('\varepsilon _{t,max}', abs(et_max_val), '',
                          'Maximimum tensile strain in reinforcement steel')
    bars_out = CalcTable('ReinforcementResults', bars_output, result_check=True)

    bar_moment = CalcVariable('\Sigma A_{si} f_{si} y_{si}', bars_sum, 'k-in',
                              'Net moment contribution from reinforcement steel')
    conc_moment = CalcVariable('M_{nc}', 0.85 * (fc / 1000) * b * B1 * c * (B1 * c) / 2, 'k-in',
                               'Net moment contribution from concrete compression')

    Mn = CalcVariable('M_n', bar_moment - conc_moment, 'k-in', 'Nominal moment capacity of beam section',
                      result_check=False)

    phi = CalcVariable('\phi', MAX(0.65, MIN(0.9, 0.65 + 0.25 * BRACKETS((et_max - ey) / (0.005 - ey)))), '',
                       'Strength reduction factor', code_ref='Table 21.2.2')

    phi_mn = CalcVariable('\phi M_n', phi * Mn / ft_to_in, 'k-ft', 'Design moment capacity of beam section',
                          result_check=True)
