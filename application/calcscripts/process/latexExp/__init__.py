...
#
#
# UPDATED BY ANDREW YOUNG FOR PYTHON 3 COMPATIBILITY AND MINOR UPDATES REQUIRED FOR ENCOMP APPLICATION:
...
class Variable(object):
	...
	name = ''                   #: symbolic name
	unit = ''                   #: physical unit
	format = '%.4g  '             #: string to be formatted by the numeric value (with '%' operation)
	...
	def strSubstituted(self):
		...
		.. code-block:: python
			...
			3.45
		"""
		return self.strResultWithUnit()  #  strResultWithUnit
...
# Variable.__xor__ = Variable.__pow__
...
_supportedOperations = _supportedOperations1 + _supportedOperations2 + _supportedOperationsN
...
class Operation(object):
	...
	def __str(self,what):
		...
		if t in _supportedOperationsN:
			...
			if t == _ADD: return r' + '.join(v)
			if t == _MUL: return r' \cdot '.join(v)
		...
		if t in _supportedOperations2:
			v0 = getattr(a[0],what)()
			v1 = getattr(a[1],what)()
			if t == _SUB:  return r'%s - %s'%(v0,v1)
			if t == _DIV:  return r'\frac{ %s }{ %s }'%(v0,v1)
			if t == _DIV2: return r'%s / %s'%(v0,v1)
			if t == _POW:  return r'{\left( %s \right)}^{ %s }'%(v0,v1)
		...
		if t in _supportedOperations1:
			...
			if t == _POS:   return r'\left( + %s \right)'%v
			if t == _ABS:   return r'\left| %s \right|'%v
			if t == _SQR:   return r'%s^2'%v
			if t == _SQRT:  return r'\sqrt{ %s }'%v
			if t == _SIN:   return r'\sin{\left( %s \right)}'%v
			if t == _COS:   return r'\cos{\left( %s \right)}'%v
			if t == _TAN:   return r'\tan{\left( %s \right)}'%v
			if t == _SINH:  return r'\sinh{\left( %s \right)}'%v
			if t == _COSH:  return r'\cosh{\left( %s \right)}'%v
			if t == _TANH:  return r'\tanh{\left( %s \right)}'%v
	...
	def result(self):
		...
		if t in _supportedOperationsN:
			...
			if t == _ADD: return sum(v)
			if t == _MUL: return reduce(lambda x,y: x*y, v, 1.)
...
# Operation.__xor__ = Operation.__pow__
...
class Expression(object):
	...
	def __init__(self,name,operation,unit="",format='%.4g',unitFormat=r'\mathrm{%s}',exponent=0):
		self.name = name
		self.operation = Operation(_NONE,operation) if isinstance(operation,Variable) else operation
		self.unit = unit
		self.format = format
		self.unitFormat = unitFormat
		self.exponent = exponent
...
# Expression.__xor__ = Expression.__pow__
